# =============================================================================
# DATA CLEANING AND PROCESSING SCRIPT - DECOUPLING ANALYSIS
# Project: Decoupling Analysis of Asian Economies (1973-2023)
# Description: This script processes EDGAR GHG emissions data and GDP data
#              to prepare datasets for Tapio decoupling analysis.
# =============================================================================
...
print(unique(emissions_processed$Country))
...
# Define target countries using World Bank naming
target_countries_gdp <- c("China", "India", "Japan", "Korea, Rep.", "Indonesia")

gdp_processed <- gdp_raw %>%
  # Select only Country and year columns (1973-2023)
  select(Country, matches("^(197[3-9]|198\\d|199\\d|200\\d|201\\d|202[0-3])")) %>%
  # Filter for target countries (using World Bank names)
  filter(Country %in% target_countries_gdp)
...
# 5.4 Convert country names to match emissions dataset
gdp_long <- gdp_long %>%
  mutate(
    Country = case_when(
...
# Verify final country names
print(unique(gdp_long$Country))
